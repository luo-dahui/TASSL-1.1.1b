...
During normal operation OpenSSL (libtacrypto) will allocate various resources at
...
libtacrypto (see also L<OPENSSL_init_ssl(3)> for the libtassl
equivalent).
...
The B<opts> parameter specifies which aspects of libtacrypto should be
...
Suppress automatic loading of the libtacrypto error strings. This option is
...
Automatic loading of the libtacrypto error strings. With this option the
library will automatically load the libtacrypto error strings.
...
libtacrypto ciphers. This option is a default option. Once selected subsequent
...
With this option the library will automatically load and make available all
libtacrypto digests. This option is a default option. Once selected subsequent
calls to OPENSSL_init_crypto() with the option
B<OPENSSL_INIT_NO_ADD_ALL_CIPHERS> will be ignored.

=item OPENSSL_INIT_NO_ADD_ALL_CIPHERS

With this option the library will suppress automatic loading of libtacrypto
...
used by calling OPENSSL_config(). This is not a default option for libtacrypto.
As of OpenSSL 1.1.1 this is a default option for libtassl (see
L<OPENSSL_init_ssl(3)> for further details about libtassl initialisation). See the
...
With this option the library with automatically initialise the libtacrypto async
...
The OPENSSL_cleanup() function deinitialises OpenSSL (both libtacrypto
and libtassl). All resources allocated by OpenSSL are freed. Typically there
