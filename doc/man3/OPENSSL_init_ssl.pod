...
OPENSSL_init_ssl - OpenSSL (libtassl and libtacrypto) initialisation
...
During normal operation OpenSSL (libtassl and libtacrypto) will allocate various
...
this function will explicitly initialise BOTH libtacrypto and libtassl. To
explicitly initialise ONLY libtacrypto see the
...
The B<opts> parameter specifies which aspects of libtassl and libtacrypto should be
initialised. Valid options for libtacrypto are described on the
L<OPENSSL_init_crypto(3)> page. In addition to any libtacrypto
specific option the following libtassl options can also be used:

=over 4

=item OPENSSL_INIT_NO_LOAD_SSL_STRINGS

Suppress automatic loading of the libtassl error strings. This option is
...
Automatic loading of the libtassl error strings. This option is a
